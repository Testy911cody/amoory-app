#!/usr/bin/env node
/** Verify More words count — tier 0 vs full vocabulary. */
...
import { getUnlockedTier } from "../public/src/usage.js";
...
const visible = allFlat.filter(w => w.tier <= unlocked);
const home = computeHomeWords(visible, []);
...
console.log("Unlocked tier:", unlocked);
console.log("Tier-visible (old pool):", visible.length);
console.log("Home words:", home.length);
console.log("More words (fixed):", more.length);
